-- Migration 014: Coluna is_ineligible em google_maps_signals
-- Marca registros com categoria fora do ICP (barbearia, academia, escola, etc.)
-- Para esses, análise manual posterior decide o que fazer.
-- Por padrão false — marcação feita manualmente via Supabase.

ALTER TABLE google_maps_signals
  ADD COLUMN IF NOT EXISTS is_ineligible BOOLEAN NOT NULL DEFAULT false;

-- Recria pick_next_clinic() excluindo is_ineligible E is_franchise

-- Migration 013: Coluna is_franchise em google_maps_signals
-- Marca redes/franquias que compartilham o mesmo número WhatsApp
-- Para essas clínicas, o SDR não aborda automaticamente — contato manual

-- 1. Adiciona coluna
ALTER TABLE google_maps_signals
  ADD COLUMN IF NOT EXISTS is_franchise BOOLEAN NOT NULL DEFAULT false;

-- 2. Marca como franquia todos os registros cujo phone_e164 aparece
--    mais de uma vez (múltiplas unidades com o mesmo número central)
UPDATE google_maps_signals
SET is_franchise = true
WHERE phone_e164 IN (
  SELECT phone_e164
  FROM google_maps_signals
  WHERE phone_e164 IS NOT NULL AND phone_e164 != ''
  GROUP BY phone_e164
  HAVING COUNT(*) > 1
);

-- Também marca duplicatas pelo campo phone (captura casos não normalizados)
UPDATE google_maps_signals
SET is_franchise = true
WHERE phone IN (
  SELECT phone
...
-- 3. Recria pick_next_clinic() excluindo franquias do pool de seleção
...
BEGIN
    ...
    FROM (
        ...
        WHERE gas.ads_count > 0
          AND (gms.is_franchise IS NOT TRUE)   -- exclui franquias
        GROUP BY grp
    ) st;
